(ns aoc-clj.year-2024.day23-test
...
(deftest three-node-networks
  (testing "Finds all of the three-node networks"
    (is (= #{#{"aq" "cg" "yn"}
             ...
             #{"td" "wh" "yn"}
             #{"ub" "vc" "wq"}}
           (d23/three-node-networks d23-s00)))))
...
(deftest largest-clique-test
  (testing "Finds the largest subnetwork of mutually interconnected nodes"
    (is (= #{"co" "de" "ka" "ta"}
           (d23/largest-clique d23-s00)))))
...
(deftest part2-test
  (testing "Reproduces the answer for day23, part2"
    (is (= "az,cg,ei,hz,jc,km,kt,mv,sv,sx,wc,wq,xy"
           (d23/part2 day23-input)))))