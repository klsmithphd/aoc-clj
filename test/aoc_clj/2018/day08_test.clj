...
(deftest add-node-test
  (testing "Constructs the tree node by node"
    (is (= {:children [] :metadata []} (d08/add-node [0 0])))
    (is (= {:children [] :metadata [2]} (d08/add-node [0 1 2])))
    (is (= {:children
            [{:children [] :metadata []}]
            :metadata []} (d08/add-node [1 0 0 0])))
    (is (= {:children
            [{:children
              [{:children [] :metadata []}] :metadata []}]
            :metadata []} (d08/add-node [1 0 1 0 0 0])))
...
(deftest metadata-sum-test
  (testing "Computes the sum of all the metadata in the tree"
    ...
    (is (= 20 (d08/metadata-sum {:children
                                 ...
                                 :metadata [4 6]})))
    (is (= 138 (d08/metadata-sum (d08/add-node d08-s00))))))
