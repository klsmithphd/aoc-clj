...
(deftest step-test
  (testing "Evolves the map one step at a time, moving all blizzards to their
    ...
    (is (= {:x-bound 5, :y-bound 5, :blizzards '([[4 5] :d] [[2 2] :r])}
           (t/state-at-t d24-s01 1)))
    ;; Bottommost blizzard wraps around to top, leftmost blizzard moves right
    (is (= {:x-bound 5, :y-bound 5, :blizzards '([[4 1] :d] [[3 2] :r])}
           (t/state-at-t d24-s01 2)))
    ;; Blizzards now occupy same space
    (is (= {:x-bound 5, :y-bound 5, :blizzards '([[4 2] :d] [[4 2] :r])}
           (t/state-at-t d24-s01 3)))
    ;; Horizontal blizzard reaches right wall, vertical blizzard moves down
    (is (= {:x-bound 5, :y-bound 5, :blizzards '([[4 3] :d] [[5 2] :r])}
           (t/state-at-t d24-s01 4)))
    ;; Horizontal blizzard wraps around to left wall
    (is (= {:x-bound 5, :y-bound 5, :blizzards '([[4 4] :d] [[1 2] :r])}
           (t/state-at-t d24-s01 5)))))
